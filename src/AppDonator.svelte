<script>
    ...
    let { username, idx } = $props();

    let loading = $state(true);
    let data = $state({});
    let tmpdata = $state({});
    let message = $state("Error");
    let selectedFile = $state(undefined);
    ...
    async function getData(session, diary = false, offset = 0) {
        let url =
            baseUrl + "donator" + "?session=" + session + "&offset=" + offset;
        if (diary) {
            url += "&diary";
        }
    ...
    onMount(async () => {
        ...
        if (
            ...
            localStorageData !== "undefined" &&
            localStorageData.toLowerCase() === username.toLowerCase()
        ) {
        ...
        try {
            ...
            const watchPromises = Array.from(
                { length: Math.ceil(watches / limit) },
                (_, i) => getData(session, false, i),
            );
            const diaryPromises = Array.from(
                { length: Math.ceil(diary / limit) },
                (_, i) => getData(session, true, i),
            );
            const [watchResults, diaryResults] = await Promise.all([
                Promise.all(watchPromises),
                Promise.all(diaryPromises),
            ]);
            watchResults.forEach((tmpdata) => {
                dataWatched = dataWatched.concat(tmpdata.watched || []);
            });
            diaryResults.forEach((tmpdata) => {
                dataDiary = dataDiary.concat(tmpdata.diary || []);
            });
            ...
            message =
                "Error, make sure you are a supporter and contact the developer";
        }
        loading = false;
    });
</script>
